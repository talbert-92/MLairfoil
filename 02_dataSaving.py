...
for fn in polar_files:
    ...
    # The script expects filenames based on the new convention, such as:
    # polar_NACA4_NACA2412_Re300000.txt
    ...
    # The part after "polar_" and before "_Re" is the full airfoil name
    airfoil_parts = []
    re_part = None
    # Start from index 1 to skip "polar"
    for part in parts[1:]:
        if "Re" in part:
            re_part = part
            break
    ...
    if re_part is None:
        print(f"Warning: No Reynolds number found in filename: {basename}. Using Re=0.")
        re_value = 0
    else:
        try:
            ...
            print(f"Warning: Could not parse Reynolds number from {re_part} in {basename}. Using Re=0.")
            re_value = 0
    ...
    try:
        ...
        temp_df["airfoil"] = airfoil # This will be the composite name, e.g., "NACA4_NACA2412"
        temp_df["Re"] = re_value
...
for foil in airfoils_used:
    # Construct path to .dat file using the composite name (e.g., "NACA4_NACA2412.dat")
    dat_path = os.path.join(airfoil_folder, f"{foil}.dat")
    
    if not os.path.isfile(dat_path):
        print(f"  • Skipping geometry for '{foil}': .dat file not found at {dat_path}")
        geo_feats.append({'airfoil': foil, 'max_thickness': np.nan, 'x_thickness': np.nan, 'max_camber': np.nan, 'x_camber': np.nan, 'le_radius': np.nan})
        continue
...
# 5. *** NEW STEP: Split airfoil name into family and simple name ***
print("Splitting airfoil name into family and simple name...")
df_split = master_df['airfoil'].str.split('_', n=1, expand=True)

if df_split.shape[1] == 2:
    master_df['airfoilFamily'] = df_split[0]
    master_df['airfoil'] = df_split[1]
    print("  • Successfully created 'airfoilFamily' column.")
else:
    master_df['airfoilFamily'] = 'Unknown'
    master_df['airfoil'] = df_split[0]
    print("  • Warning: No '_' found in airfoil names. 'airfoilFamily' set to 'Unknown'.")

# Reorder columns to place airfoilFamily before airfoil
cols = master_df.columns.tolist()
cols.insert(cols.index('airfoil'), cols.pop(cols.index('airfoilFamily')))
...
try:
    with conn:
        ...
        # *** MODIFIED: Added airfoilFamily column ***
        conn.execute("""
            CREATE TABLE polars (
                airfoilFamily TEXT,
                airfoil TEXT,
        ...
        """)
        conn.execute("CREATE INDEX idx_airfoil_Re ON polars(airfoilFamily, airfoil, Re);")
    ...
    cursor.execute("SELECT COUNT(DISTINCT airfoilFamily) FROM polars")
    unique_families = cursor.fetchone()[0]
    
    print(f"Database contains:")
    print(f"  • {total_rows:,} total data points")
    print(f"  • {unique_families} unique airfoil families")
